 ...
 */
export async function runInterruptFollowupTimeoutClarification(config, ctx, runtime) {
  ...
  try {
    dialogue = await runLiveDialogueOnCallerTranscript(config, ctx, runtime, candidate);
  } catch (err) {
    dialogue = { ok: false, reason: "dialogue_exception", error: String(err?.message ?? err) };
  }
  ...
  if (dialogue?.ok) {
    ...
    try {
      ...
      if (playback?.ok) {
        markInterruptFollowupLatency(runtime, "followup_playback_started", Date.now());
      }
    } catch (err) {
