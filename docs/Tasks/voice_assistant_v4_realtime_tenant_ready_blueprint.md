...
Artifact: `voice-bridge/config/playbooks/technolohit.main_voice_sales.v1.json` — **draft, manually maintained, not runtime-active** (`runtime_binding.active=false`; existing agent_config remains the runtime source of truth) — [report](./voice_assistant_v4_phase10am_technolohit_structured_playbook_report.md).

- [x] Successful: minimal schema defined (`tenant-playbook-1`; required fields validated by non-runtime `src/v4/playbook-loader.js` in tests only).
- [x] Successful: Smart Website included.
- [x] Successful: AI Voice Agent included (Digitale Rezeption / `voice_agent`).
- [x] Successful: LokalKI included.
- [x] Successful: pricing policy included (no invented fixed prices; scope-dependent).
- [x] Successful: closing policy included (Phase 10AK phrases, contract response, priority/overrides, context-sensitive "Stopp").
- [x] Successful: role-boundary policy included (no general chatbot behavior; disallowed topics).
- [x] Successful: lead capture policy included (answer first; capture only when appropriate).
- [x] Successful: QA criteria included.
- [x] Successful: playbook remains manually maintained first (draft status; approval metadata; no runtime wiring).
- [x] Successful: initial eval scenarios included (closing, out-of-scope, technical escalation, pricing, callback, product/combined inquiry, interruption).

#### Phase 10AN - Playbook-Driven Runtime Increment 1

Goal: move only the safest repeated behavior rules into config/playbook.

Status: **implemented** via guarded resolver `voice-bridge/src/v4/behavior-policy.js` — opt-in (`VOICE_V4_PLAYBOOK_RUNTIME_ENABLED`, default **false**), fail-closed, equivalence-tested against Phase 10AK — [report](./voice_assistant_v4_phase10an_playbook_runtime_increment1_report.md). **Phase 10AN introduces a guarded resolver and tests only for safest repeated behavior. It does not make the draft playbook production-active.**

Candidate first fields:

- [x] Successful: closing phrases (playbook may extend, never replace, the hardcoded 10AK detection; wired into v4 intent detection).
- [x] Successful: closing response (wired into v4 closing plan; hardcoded contract text with default env).
- [x] Successful: out-of-scope redirect (resolver accessor provided; not yet wired to a runtime path).
- [x] Successful: safe escalation response (resolver accessor provided; not yet wired to a runtime path).
- [x] Successful: fallback clarification response (wired into the generic v4 fallback plan; hardcoded text with default env).
- [ ] Successful: product answer length target (deferred — not in increment 1).
- [ ] Successful: pricing wording (deferred — not in increment 1).
- [ ] Successful: lead capture soft prompt (deferred — not in increment 1).
...
Status: **implemented** — non-live eval runner `voice-bridge/src/v4/playbook-eval-scenarios.js` loads `eval_scenarios` from the draft playbook, runs all categories through the planner/orchestrator harness (Phase 10AP wired pending categories), and stores privacy-safe results keyed by `playbook_version` — [report](./voice_assistant_v4_phase10ao_playbook_eval_scenarios_report.md). **Does not make the draft playbook production-active.** See also [Phase 10AP report](./voice_assistant_v4_phase10ap_runtime_consumers_for_role_boundary_report.md) for runtime consumers.
...
- [x] Successful: out-of-scope scenarios added (runtime pass via role-boundary redirect — Phase 10AP).
- [x] Successful: technical uncertain scenarios added (runtime pass via technical escalation — Phase 10AP).
- [x] Successful: product/pricing scenarios added (planner path).
- [x] Successful: callback request scenarios added (runtime pass via safe lead-capture path — Phase 10AP).
...
#### Phase 10AP - Runtime Consumers For Pending Playbook Eval Categories

Goal: wire minimal v4 runtime consumers for out-of-scope redirect, technical/uncertain escalation, and callback/lead-capture intent so playbook eval scenarios move from pending to pass.

Status: **implemented** — `role-boundary-intent.js`, planner/orchestrator wiring, RAG gate, eval runner updates; all nine playbook eval scenarios pass with hardcoded defaults — [report](./voice_assistant_v4_phase10ap_runtime_consumers_for_role_boundary_report.md). **Does not enable production v4 or make the draft playbook production-active.**

- [x] Successful: out-of-scope redirect (no RAG, no lead, no general-knowledge answer).
- [x] Successful: technical/uncertain escalation (no overpromise; not callback-ready by itself).
- [x] Successful: callback/lead-capture intent routes to safe contact-preference flow; validator unchanged.
- [x] Successful: closing (#1) still overrides all three paths.
- [x] Successful: playbook policy text only when opt-in policy injected; default hardcoded equivalence with flag off.
...
#### Phase 10AR - Opt-In v4 Questionnaire Runtime Wiring After Product Answers

Goal: wire Phase 10AQ generator into v4 planner/orchestrator behind explicit opt-in flag, only after safe product/pricing answers.

Status: **implemented** — `questionnaire-runtime.js`, planner wrapper, orchestrator quality fields; `VOICE_V4_QUESTIONNAIRE_RUNTIME_ENABLED=false` default — [report](./voice_assistant_v4_phase10ar_questionnaire_runtime_wiring_report.md). **Does not enable production v4 globally.**

- [x] Successful: explicit flag `VOICE_V4_QUESTIONNAIRE_RUNTIME_ENABLED` (default false).
- [x] Successful: runtime eligibility gate (v4 path active, product/pricing answer, block conditions).
- [x] Successful: at most one soft follow-up; append to spoken text only within TTS limit.
- [x] Successful: `follow_up_question` + `questionnaire` plan evidence; safe quality payload fields.
...
- [x] Partial: supervised `voice-bridge-v1.35.1` canary — callback request and "telefonisch" now plan correctly, but "Ja." after the permission question regressed to `scoped_product_qa` and live RAG made only 1 attempt (handed to Phase 10AT).

#### Phase 10AT - Callback Permission Continuation + Live RAG Transient Retry

Goal: fix the `v1.35.1` supervised canary failures — (A) "Ja." after `collect_callback_permission` must grant callback permission instead of returning to scoped product Q&A; (B) live RAG must honor `VOICE_RAG_RETRIEVE_MAX_ATTEMPTS`/`VOICE_RAG_RETRIEVE_TIMEOUT_MS` by retrying transient failures, not only timeouts.

Status: **implemented, not live-accepted yet** — [report](./voice_assistant_v4_phase10at_callback_permission_and_live_rag_retry_report.md).

- [x] Successful: `isCallbackPermissionPending()` + grant/refusal intents ("ja", "ja gerne", "okay", "einverstanden" grant; "nein"/"bitte nicht" denies) robust against live state churn.
- [x] Successful: contact-flow continuation intents skip the scoped-product-QA branch in `buildResponsePlanCore()`; "Ja." plans `callback_permission_granted`, never `scoped_product_qa`.
- [x] Successful: refusal plans `callback_permission_denied` (polite e-mail alternative, `lead_ready=false`, no callback-ready lead, no product Q&A).
- [x] Successful: RAG gate and questionnaire runtime block all callback/contact continuation intents.
- [x] Successful: post-call summary never stores acknowledgement-only text ("Ja.") as caller need.
...
**Status: Phases 10A–10Y implemented; 10AA/10AB Gate 2 combined inquiry PASS on v1.34.3; 10AC–10AG address Gate 3 RAG fallback, preflight, diagnostics, live timeout retry, and retrieve-specific timeout config; production v4 NOT globally enabled.**
...
- [x] Successful: Phase 10M `live_call_quality_summary` persistence fix (UUID validation), turn latency metrics, German goodbye handling — report [phase10m](./voice_assistant_v4_phase10m_live_summary_latency_closing_report.md).
