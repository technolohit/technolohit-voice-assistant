...
| Item | Status |
|------|--------|
| **Current completed phase** | **Phase 10Q — hard single-stop barge-in reliability (post 10P canary failure)** |
| **Current production runtime** | **v3** (`VOICE_RUNTIME_VERSION=v3`); canary baseline **`voice-bridge-v1.25.0`**; fix target **`voice-bridge-v1.27.0`** |
| **v4 live canary** | **v1.25.0 single-call PASS**; **10O-A failed on v1.26.0** (double Stop/Stopp); **10Q** addresses single-stop reliability |
| **v4 production status** | **Not globally enabled** |
| **Phase 9 dry run** | **Passed** (2026-06-01) |
| **Next step** | Re-run **10O-A** on v1.27.0+; **10O-B RAG-on blocked** — [10Q report](./voice_assistant_v4_phase10q_single_stop_reliability_report.md) |
...
**Status: Phases 10A–10Q implemented; v1.25.0 single canary PASS; 10O-A repeatability failed on v1.26.0; 10Q single-stop fix pending re-validation; production v4 NOT globally enabled.**
...
- [x] Successful: Phase 10P interrupt listen window, marker-only defer, closed-domain intent, interrupt latency metrics — [report](./voice_assistant_v4_phase10p_turn_taking_interruption_wait_report.md). **10O-A still failed on v1.26.0** (double Stop/Stopp).
- [x] Successful: Phase 10Q hard single-stop reliability — utterance preserve on barge-in, marker split, follow-up quality events, timing metrics — [report](./voice_assistant_v4_phase10q_single_stop_reliability_report.md).
- [ ] Successful: Phase 10O-A — 3/3 repeatability (RAG off) on v1.27.0+ — **failed on v1.25.0/v1.26.0; retry after 10Q**.
...
- Phase 10 live AudioSocket wiring: **10Q single-stop reliability** shipped in repo; **10O-A failed** on v1.26.0; re-validate on v1.27.0+; **10O-B RAG blocked**; production v4 **not globally enabled**.
