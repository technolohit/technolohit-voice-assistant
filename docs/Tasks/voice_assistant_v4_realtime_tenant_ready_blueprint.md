...
| Item | Status |
|------|--------|
| **Current completed phase** | **Phase 10R — repeated/nested interruption stability (post 10Q partial canary)** |
| **Current production runtime** | **v3** (`VOICE_RUNTIME_VERSION=v3`); canary baseline **`voice-bridge-v1.25.0`**; fix target **`voice-bridge-v1.28.0`** |
| **v4 live canary** | **v1.27.0 PARTIAL** (single stop improved); **10R** addresses repeated/nested interrupts |
| **v4 production status** | **Not globally enabled** |
| **Phase 9 dry run** | **Passed** (2026-06-01) |
| **Next step** | Re-run **10O-A** on v1.28.0+; **10O-B RAG-on blocked** — [10R report](./voice_assistant_v4_phase10r_repeated_interruption_stability_report.md) |
...
**Status: Phases 10A–10R implemented; v1.25.0 single canary PASS; 10O-A failed on v1.26.0/v1.27.0; 10Q partial; 10R repeated-interrupt fix pending re-validation; production v4 NOT globally enabled.**
...
- [x] Successful: Phase 10Q hard single-stop reliability — [report](./voice_assistant_v4_phase10q_single_stop_reliability_report.md). **v1.27.0 canary PARTIAL** (first stop improved; repeated interrupts still weak).
- [x] Successful: Phase 10R repeated/nested interruption stability — cycle reset, stale recovery clear, timing privacy exempt — [report](./voice_assistant_v4_phase10r_repeated_interruption_stability_report.md).
- [ ] Successful: Phase 10O-A — 3/3 repeatability (RAG off) on v1.28.0+ — **failed on v1.25.0–v1.27.0; retry after 10R**.
...
- Phase 10 live AudioSocket wiring: **10R repeated-interrupt stability** shipped in repo; **10O-A failed** through v1.27.0; re-validate on v1.28.0+; **10O-B RAG blocked**; production v4 **not globally enabled**.
